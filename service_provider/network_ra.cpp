...
/*
int ra_network_send_receive(const char *server_url,const ra_samp_request_header_t *p_req,ra_samp_response_header_t **p_resp)//ra_samp_response_header_t **p_resp NULL pass hocche isv_app theke
{
    int ret = 0;
    ra_samp_response_header_t* p_resp_msg;//eta age kokono define hoy nai tai ekane define korci
	 
   if((NULL == server_url) ||(NULL == p_req) ||(NULL == p_resp))//na likeleo cholbe
   {
       return -1;
     }
    ...
    case TYPE_RA_MSG0:
        ret = sp_ra_proc_msg0_req((const sample_ra_msg0_t*)((size_t)p_req + sizeof(ra_samp_request_header_t)), p_req->size);//sp_ra_proc_msg0_req define in service_provider.cpp
        if (0 != ret)
        {
