...
typedef struct sample_ra_msg2_t  //pg407
{
    ...
    sample_mac_t                mac;        // mac_smk(g_b||spid||quote_type||sign_gb_ga)
...
typedef int (*sample_enroll)(int sp_credentials, sample_spid_t* spid, int* authentication_token);

typedef int(*sample_get_sigrl)(const sample_epid_group_id_t gid, uint32_t* p_sig_rl_size,  uint8_t** p_sig_rl);

typedef int(*sample_verify_attestation_evidence)(const sample_quote_t* p_isv_quote,  uint8_t* pse_manifest,  ias_att_report_t* attestation_verification_report);
